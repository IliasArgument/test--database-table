...
getFilteredData () {
   const {date, search} = this.state;
     ...
     return date.filter(item => {
      ...
      item['lastName'].toLowerCase().includes(search.toLowerCase()) || item['email'].toLowerCase().includes(search.toLowerCase())
     })
}
  ...
  render() {
    ...
    <div>
   {
      ...
      ?
      <div style={{display: 'flex', justifyContent: 'center', padding: 100}}>'...Loading' </div>
      : 
      <React.Fragment>
      ...
      }
      {
        this.state.row 
        ? 
        <DetailsRow person={this.state.row}/>
        :
        null
      }
      </div>
